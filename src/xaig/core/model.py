...
def _sort_key(value: AttrValue) -> tuple[int, int | float, str]:
    """Numbers numerically, then text, then missing: batch 8 sorts before 16.

    A number is compared as it is, never through ``float``: Python orders an int
    against a float exactly, while a float holds integers only up to 2**53 -- short
    of a job id or a timestamp in nanoseconds, which would then sort as equals."""
    if value is None:
        return (2, 0, "")
...
class Campaign:
    ...
    def sorted_by(self, *keys: str) -> Campaign:
        def sort_key(r: Run) -> tuple[tuple[int, int | float, str], ...]:
