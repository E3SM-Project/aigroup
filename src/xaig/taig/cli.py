"""``xaig taig``: train the toy architectures. Must import on a base install, so
...
def taig() -> None:
    """Train small reusable architectures on a model's latents."""
...
def sae_cmd(source, adapter, mask_variable, layer, out, times, k, activation, **kw) -> None:
    ...
    `latent series --basis`, `latent fields --basis`, and the web app.
    """
