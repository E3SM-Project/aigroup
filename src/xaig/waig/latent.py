...
def _analyse(path: str, mask_variable: str | None, settings: dict) -> RegionAnalysis:
    ...
    return analyse_region(
        ...
        region=region,
        basis=_basis(basis) if basis else None,
        **kwargs,
    )
...
def _series(
    path: str, mask_variable: str | None, layer: int, region, columns, centred, basis
) -> RegionSeries:
    chosen = {"features": columns, "basis": _basis(basis)} if basis else {"channels": columns}
...
def reproduction(path: str, mask_variable: str | None, settings: dict) -> tuple[str, str]:
    ...
    arguments += [f"{k}={v!r}" for k, v in s.items() if k not in ("region", "basis")]
