...
def analyse_region(
    ...
    *,
    ...
    respond most strongly in the region, by peak absolute activation there; the
    basis sees the raw latents whatever ``centred`` says, since it carries the
    standardisation it was fitted with.
    ...
    if basis is not None and basis.n_channels != width:
        raise RequestError(
    ...
    if n_components and basis is not None:
        local = np.abs(basis.transform(latents[nodes]))
        peak = np.nan_to_num(local.max(axis=0), nan=-np.inf)
        features = np.argsort(peak, kind="stable")[::-1][:n_components]
        scores = basis.transform(latents, features=features)
        feature_info = _feature_info(basis, features, "F", peak_abs=peak[features])
    if centred:
        centre(latents)
    if ranking is None:
