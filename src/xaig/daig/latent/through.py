...
from xaig.daig.latent.source import LatentSource, check_comparable
...
class PairedDifference:
    ...
    for the ranked channels, ``(n_nodes, len(ranking.channels))``, NaN where the
    grid is invalid.
    """
...
def difference(
    ...
    *,
    ...
    check_comparable(control, experiment, layer=layer)
...
def difference_growth(
    ...
    *,
    ...
    ``experiment - control`` at every layer and every time the two share."""
...
def correlate_field(
    ...
    first = np.zeros(latents.shape[1])
    square = np.zeros(latents.shape[1])
    cross = np.zeros(latents.shape[1])
    for start in range(0, keep.size, _BLOCK):
        rows = keep[start : start + _BLOCK]
        block = latents[rows].astype(np.float64)
        wb = w[start : start + _BLOCK]
        first += wb @ block
        square += wb @ (block * block)
        cross += (wb * y[start : start + _BLOCK]) @ block
    spread = np.clip(square - first**2, 0.0, None)
    # "Does not vary" to rounding, not to the last bit: a constant channel's
    # variance comes out as 1e-16 of its square, and would correlate at random.
    varies = (spread > 1e-10 * square) & (w @ (y * y) > 0.0)
    out = np.full(latents.shape[1], np.nan)
    np.divide(cross, np.sqrt(spread * (w @ (y * y))), out=out, where=varies)
...
def rank_by_field(
    ...
    *,
    ...
    if basis is None:
        ...
        if basis.n_channels != latents.shape[1]:
            raise RequestError(
                f"the basis reads {basis.n_channels} channel(s), and layer {layer} "
                f"has {latents.shape[1]}"
            )
