...
def test_numbers_sort_exactly_beyond_what_a_float_holds():
    """A nanosecond timestamp is past 2**53, where neighbouring integers share a float."""
    big = 2**53
    c = Campaign("c", tuple(Run(str(i), {"n": n}) for i, n in enumerate((big + 1, big, 2.5))))
    assert [r.attrs["n"] for r in c.sorted_by("n")] == [2.5, big, big + 1]
