...
Same as the [vanilla guide](ace2-workflow.md#training-parameters), with the additions:

- `--nnodes="$SLURM_JOB_NUM_NODES"`: number of nodes in the allocation
- `--rdzv_backend=c10d` / `--rdzv_endpoint=...`: multi-node rendezvous on the head node
- `FME_DISTRIBUTED_BACKEND=model`, `FME_DISTRIBUTED_H`, `FME_DISTRIBUTED_W`: enable and shape the spatial decomposition

## Monitoring Training

Same as the vanilla workflow:

- GPU utilization: `nvidia-smi` (run on each node as needed)
- Training logs: under the `experiment_dir` set in your config
- Checkpoints: written periodically based on your configuration

## Troubleshooting

In addition to the [common issues from the vanilla guide](ace2-workflow.md#common-issues), spatial-decomposition runs have a few extra pitfalls:

**`KeyError: 'FME_DISTRIBUTED_H'` (or `_W`)**

You set `FME_DISTRIBUTED_BACKEND=model` but did not export both `FME_DISTRIBUTED_H` and `FME_DISTRIBUTED_W`. Set them and re-launch.

**`Unknown FME_DISTRIBUTED_BACKEND value`**

Only `torch`, `model`, and `none` are accepted. Check for typos.

**Hang at startup or rendezvous timeout**

- Confirm `HEAD_NODE` resolves to a hostname inside the allocation (`scontrol show hostnames "$SLURM_JOB_NODELIST"` should list it).
- Make sure the rendezvous port (`29500` in the example) is not in use; pick another if needed.
- Verify `--nnodes` matches `$SLURM_JOB_NUM_NODES` and that `srun` is fanning out to every node.

**Batch-size / divisibility error**

If FME complains about an indivisible batch, recompute `DATA_DIM = NPROCS / (H * W)` and make sure `train_loader.batch_size % DATA_DIM == 0`.

**Out-of-memory errors with spatial decomposition**

- Increase `H` and/or `W` to give each rank a smaller spatial tile (remember to also re-check the sizing constraints).
- Reduce model size (`embed_dim`, `num_layers`).
- Reduce `batch_size`.

## Additional Resources

- [Vanilla ACE2-ERA5 Training Workflow](ace2-workflow.md)
