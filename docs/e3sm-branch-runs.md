...
In training and evaluating emulators for E3SM, we often need
specialized output datasets that standard runs do not produce.
In this quick guide, the a procedure is documented for how to
extend or repeat a segment of a concluded run to produce additional
specialized outputs, potentially maintaining identical results
between the old and new run (depending on how old the old one is).

## Prerequisites

- An old run or a description thereof (i.e., one needs to know
what they are up against) --- ideally, the run script and associated
data files needed
...
Consider the `run.v3.LR.historical_0201.sh.20240522-125553` ([below](#scripts))
for an old run we want to replicate.
We can modify this very script to accomplish that;
see `v3.LR.historical_0201_rerun1990.sh` ([below](#scripts)).
...
- `CODE_ROOT` is unchanged here, but if unavailable, one may need
to clone the code again; `CASE_ROOT` must in the a space where the
...
- A hack to reuse the old executable, will likely ensure identical results;
like the code, it may be unavilable the user must build again
...
??? example "run.v3.LR.historical_0201.sh.20240522-125553"
    ```bash
    #!/bin/bash -fe
...
??? example "v3.LR.historical_0201_rerun1990.sh"
    ```bash
    #!/bin/bash -fe
