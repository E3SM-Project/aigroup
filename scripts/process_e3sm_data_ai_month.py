...
def process_e3sm_month(
    ...
    ds_p = compute_total_water_path(ds_p, "specific_total_water", "dp",)
...
def process_and_write_month(
    ...
    year: int,
    month: int,
    h1_files_by_month: Optional[Dict[int, List[str]]],
    ...
    try:
        ...
        forcing_output_path = os.path.join(
            forcingdata_dir, f"{year}-{month_str}.forcing.nc"
        )
        train_output_path = os.path.join(
            traindata_dir, f"{year}-{month_str}.nc"
        )
        ...
        logging.info(f"✓ Year-Month {year}-{month_str} completed successfully!")
        return (year, month, True, None)
    except Exception as e:
        logging.error(f"Error processing {year}-{month:02d}: {e}", exc_info=True)
        return (year, month, False, str(e))

def process_and_write_month_star(args):
    return process_and_write_month(*args)
...
def main(config: str, start_year: int, end_year: int, start_month: int, end_month: int, debug: bool, workers: int):
    logging.basicConfig(
    ...
    try:
        ...
        for year in range(proc_config.start_year - 1, proc_config.end_year + 1):
            ...
            if proc_config.data_paths.h0_pattern:
                try:
                    ...
                    h0_files_by_month = {}
            files_by_year[year] = (h1_files_by_month, h2_files_by_month, h0_files_by_month)
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as executor:
            results = list(
                executor.map(process_and_write_month_star, jobs)
            )

        num_success = sum(1 for _, _, ok, _ in results if ok)
        num_fail = sum(1 for _, _, ok, _ in results if not ok)
        logging.info(f"\n{'='*60}")
        logging.info(f"All months processed! {num_success} succeeded, {num_fail} failed.")
        if num_fail > 0:
            for year, month, ok, err in results:
                if not ok:
                    logging.error(f"{year}-{month:02d} failed: {err}")
        logging.info(f"{'='*60}")
